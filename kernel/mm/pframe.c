...
pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
		...
		pframe_t *pframeget = pframe_get_resident(o,pagenum);
	if (!pframeget)
	{
        dbg(DBG_PRINT, "(GRADING3B)\n");
		pframeget = pframe_alloc(o, pagenum);
		if(pframeget == NULL)
		{
        	dbg(DBG_PRINT, "(GRADING3C)\n");
			return EFAULT;
		}
		if(pframe_fill(pframeget) != 0)
		{ 
        dbg(DBG_PRINT, "(GRADING3C)\n");
			pframe_free(pframeget);
			return EFAULT;			
		}
	...
	else if(pframeget != NULL)
	{
		while(pframe_is_busy(pframeget))
		{
			sched_sleep_on(&pframeget->pf_waitq);
			pframeget = pframe_get_resident(o,pagenum);
			KASSERT(pframeget);
...
pframe_pin(pframe_t *pf)
{
        ...
        dbg(DBG_PRINT,"go into pframe_pin\n");
...
pframe_unpin(pframe_t *pf)
{
        ...
        dbg(DBG_PRINT,"go into pframe_unpin\n");
        KASSERT(!pframe_is_free(pf));
