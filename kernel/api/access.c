...
int addr_perm(struct proc *p, const void *vaddr, int perm)
{
        ...
        dbg(DBG_PRINT,"go into addr_perm\n");
    if( USER_MEM_LOW > (uint32_t) vaddr || USER_MEM_HIGH <= (uint32_t) vaddr)
    {
        dbg(DBG_PRINT, "(GRADING3C)\n");
        return 0;
    }
    vmarea_t *vma = vmmap_lookup(p->p_vmmap, ADDR_TO_PN(vaddr));
    if(vma != NULL && ((vma->vma_prot | perm)==vma->vma_prot))
    {
...
int range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
        ...
        dbg(DBG_PRINT,"go into range_perm\n");
    uint32_t vfn;
    uint32_t vfn_start =(uint32_t) avaddr, boundary = (size_t)avaddr + len ;
    for(vfn=vfn_start; vfn< boundary; vfn+=PAGE_SIZE)
    {
        if(addr_perm(p, (const void *)vfn, perm) == 0)
        {
        dbg(DBG_PRINT, "(GRADING3C)\n");
            return 0;
        }
