...
sched_sleep_on(ktqueue_t *q)
{
	...
	dbg(DBG_PRINT,"(GRADING1B) The current thread is set to sleep state\n");
...
sched_cancellable_sleep_on(ktqueue_t *q)
{
	...
	{
		dbg(DBG_PRINT,"(GRADING1C 5) The current thread state is cancelled\n");
		return -EINTR; 
	}
	curthr->kt_state= KT_SLEEP_CANCELLABLE;
	dbg(DBG_PRINT,"(GRADING1B) The current thread state is set to cancellable\n");
	ktqueue_enqueue(q,curthr);
	sched_switch();
	if(curthr->kt_cancelled == 1)
	{
	...
	dbg(DBG_PRINT,"(GRADING1B) Exiting sched_cancellable_sleep_on\n");        
	return 0;
}
...
{
	kthread_t *thr_ret=NULL;
	if(!sched_queue_empty(q))
	{
		thr_ret = ktqueue_dequeue(q);
		KASSERT((thr_ret->kt_state == KT_SLEEP) || (thr_ret->kt_state == KT_SLEEP_CANCELLABLE));
		dbg(DBG_PRINT, "(GRADING1A 4.a) Thread is either in state KT_SLEEP or in KT_SLEEP_CANCELLABLE\n");
		sched_make_runnable(thr_ret);
		return thr_ret;

	}
	else
	{
			dbg(DBG_PRINT,"(GRADING1C 1) q is empty and a thread is waiting on the q\n");		
        	return NULL;
	}
}
...
{
	dbg(DBG_PRINT,"(GRADING1B) Waking up other threads\n");
...
sched_cancel(struct kthread *kthr)
{
	...
	{
		dbg(DBG_PRINT,"(GRADING1C 5) The thread's sleep is cancellable\n");
		kthr->kt_cancelled = 1;
	...
	{
		dbg(DBG_PRINT,"(GRADING1C 5) The thread's sleep is not cancellable\n");		
		kthr->kt_cancelled = 1;
	}
}
...
sched_switch(void)
{
	...
	uint8_t interrupt_prio = intr_getipl();
	...
	dbg(DBG_PRINT,"(GRADING1B) Context switch begin\n");		
	context_switch(&thread->kt_ctx, &curthr->kt_ctx);
	intr_setipl(interrupt_prio);
...
{       
	...
	dbg(DBG_PRINT,"(GRADING1A 4.b) Thread is in run queue\n");
	
	uint8_t inter_prio = intr_getipl();
	intr_setipl(IPL_HIGH);
	thr->kt_state = KT_RUN;
	ktqueue_enqueue(&kt_runq,thr);
	intr_setipl(inter_prio);
}
