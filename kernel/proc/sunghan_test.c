...
check_sleep(char *str) {
	if (random_function() % 10 < 5) {
		dbg(DBG_PRINT, "Thread %s goes to sleep\n", str);
		sched_broadcast_on(&mynode.my_queue);
		sched_sleep_on(&mynode.my_queue);
		dbg(DBG_PRINT, "Thread %s awake\n", str);
	}
}
...
add_my_node(int arg1, void *arg2) {
	int counter = 10;
	dbg(DBG_PRINT, "Invoke add_mynode\n");
...
remove_my_node(int arg1, void *arg2) {
	int counter = 10;
	dbg(DBG_PRINT, "Invoke remove_mynode\n");
...
watch_dog(int arg1, void *arg2)
{
	while(!sched_queue_empty(&mynode.my_queue)) {
		dbg(DBG_PRINT, "Watch_dog wake up all sleeping thread\n");
		sched_broadcast_on(&mynode.my_queue);
		sched_sleep_on(&mynode.my_queue);
	}
...
sunghan_test(int arg1, void *arg2)
{
	...
	dbg(DBG_PRINT, ">>> Start running sunghan_test()...\n");
	...
	dbg(DBG_PRINT, "sunghan_test() terminated\n");
...
sunghan_deadlock_test(int arg1, void *arg2)
{
	...
	dbg(DBG_PRINT, ">>> Start running sunghan_deadlock_test()...\n");
	...
	dbg(DBG_PRINT, "Shouldn't have gotten here in sunghan_deadlock_test().  Did NOT deadlock.\n");
